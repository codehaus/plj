 ...
 *
 * IDENTIFICATION
 *	  $Header: /home/projects/plj/scm-cvs/plj2/plj-febe/src/main/java/org/codehaus/plj/febe/PGStream.java,v 1.1 2007-01-03 21:27:08 kocka Exp $
 *
 *-------------------------------------------------------------------------
 */
...
public class PGStream {
	...
	private final static Logger logger = Logger.getLogger(PGStream.class.getName());
	...
	public void SendIntegerR(int val, int siz) throws IOException {
		logger.fine("SendIntegerR -> this function should NOT be used.");
		byte[] buf = new byte[siz];
		for (int i = 0; i < siz; i++) {
			buf[i] = (byte) (val & 0xff);
			val >>= 8;
