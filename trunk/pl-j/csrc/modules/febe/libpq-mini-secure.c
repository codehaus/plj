...
#ifdef NOT_USED
static int	verify_peer(PGconn_min *);
#endif
static DH  *load_dh_file(int keylength);
static DH  *load_dh_buffer(const char *, size_t);
static DH  *tmp_dh_cb(SSL *s, int is_export, int keylength);
static int	client_cert_cb(SSL *, X509 **, EVP_PKEY **);
static int	initialize_SSL(PGconn_min *);
static void destroy_SSL(void);
static PostgresPollingStatusType open_client_SSL(PGconn_min *);
static void close_SSL(PGconn_min *);
 ...
 *	our PGconn_min * structure.  So we can't log anything!
 *
...
client_cert_cb(SSL *ssl, X509 **x509, EVP_PKEY **pkey)
{
   ...
   return 0;
	...
	char		fnbuf[2048];
	FILE	   *fp;
	PGconn	   *conn = (PGconn_min *) SSL_get_app_data(ssl);
 ...
 */
static int
initialize_SSL(PGconn_min *conn)
{
#ifndef WIN32
	struct stat buf;
	char		pwdbuf[BUFSIZ];
...
open_client_SSL(PGconn_min *conn)
{
	int			r;
