 ...
 *
 * IDENTIFICATION
 *	  $PostgreSQL: pgsql-server/src/interfaces/jdbc/org/postgresql/Driver.java.in,v 1.39 2003/12/10 18:30:44 davec Exp $
 *
 *-------------------------------------------------------------------------
 */
 ...
 *
 * @see org.pgj.jdbc.postgresql.PGConnection
 * @see java.sql.Driver
 */
public class Driver implements java.sql.Driver {
	...
	public java.sql.Connection connect(String url, Properties info)
		...
		try {
			...
			org.pgj.jdbc.postgresql.jdbc3.Jdbc3Connection con = (org.pgj.jdbc.postgresql.jdbc3.Jdbc3Connection) (Class
					.forName("org.pgj.jdbc.postgresql.jdbc3.Jdbc3Connection")
					.newInstance());
			...
			throw new PSQLException("postgresql.jvm.version",
					PSQLState.SYSTEM_ERROR, ex);
		} catch (PSQLException ex1) {
			// re-throw the exception, otherwise it will be caught next, and a
			// org.pgj.jdbc.postgresql.unusual error will be returned instead.
			throw ex1;
		} catch (Exception ex2) {
			if (Driver.logDebug) {
	...
	public static String getVersion() {
		return "PostgreSQL 7.5devel JDBC3 with SSL (build " + m_buildNumber
				+ ")";
	}
	...
	Properties parseURL(String url, Properties defaults) throws SQLException {
		...
		for (count = 0; (qst.hasMoreTokens()); count++) {
			...
			if (l_pos == -1) {
				...
				urlProps.put(token.substring(0, l_pos), token
						.substring(l_pos + 1));
			}
		}
	 ...
	 *	by specifying fully qualified class ie org.pgj.jdbc.postgresql.Driver.setLogLevel()
